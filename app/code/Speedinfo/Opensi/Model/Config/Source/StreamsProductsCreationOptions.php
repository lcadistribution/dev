<?php
/**
 * 2003-2017 OpenSi Connect
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the Open Software License (OSL 3.0)
 * that is bundled with this package in the file LICENSE.txt.
 * It is also available through the world-wide-web at this URL:
 * http://opensource.org/licenses/osl-3.0.php
 * If you did not receive a copy of the license and are unable to
 * obtain it through the world-wide-web, please send an email
 * to license@magentocommerce.com so we can send you a copy immediately.
 *
 * @category    Speedinfo
 * @package     Speedinfo_Opensi
 * @copyright   Copyright (c) 2017 Speedinfo SARL (http://www.speedinfo.fr)
 * @license     http://opensource.org/licenses/osl-3.0.php  Open Software License (OSL 3.0)
 */

namespace Speedinfo\Opensi\Model\Config\Source;

class StreamsProductsCreationOptions implements \Magento\Framework\Option\ArrayInterface
{
	/**
	 * Options getter
   *
   * @return array
   */
  public function toOptionArray()
  {
    return [
      ['value' => '', 'label' => __('--')],
      ['value' => 'brand', 'label' => __('Brand')],
      ['value' => 'short_description', 'label' => __('Short description')],
      ['value' => 'description', 'label' => __('Description')],
      ['value' => 'model_reference', 'label' => __('Model reference')],
      ['value' => 'wholesale_price', 'label' => __('Wholesale price')],
      ['value' => 'retail_price', 'label' => __('Retail price')],
      ['value' => 'ecotax', 'label' => __('Ecotax')],
      ['value' => 'barcode', 'label' => __('Barcode')],
      ['value' => 'attributes_mapping', 'label' => __('Attributes Mapping')],
      ['value' => 'volume', 'label' => __('Volume')],
      ['value' => 'height', 'label' => __('Height')],
      ['value' => 'length', 'label' => __('Length')],
      ['value' => 'width', 'label' => __('Width')],
      ['value' => 'weight', 'label' => __('Weight')],
      ['value' => 'manufacturer_reference', 'label' => __('Manufacturer reference')]
    ];
  }
}
