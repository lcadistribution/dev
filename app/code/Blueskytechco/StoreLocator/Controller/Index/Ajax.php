    ...
    public function execute()
    {
		...
		if(!$post->value){
			foreach ($locations as $item) {
				...
				if ($item->getCity() && trim($item->getCity())) {
					$html .= '<div>'.__('City').': '.$item->getCity().'</div>';
				}
				if ($item->getPostcode() && trim($item->getPostcode())) {
					$html .= '<div>'.__('Zip').': '.$item->getPostcode().'</div>';
				}
				if ($item->getCountry() && trim($item->getCountry())) {
					$html .= '<div>'.__('Country').': '.$item->getCountry().'</div>';
				}
				if ($item->getAddress() && trim($item->getAddress())) {
			...
			foreach ($locations as $item) {
				if($item->getIsActive() == 1){
					...
					$contry = $item->getCountry() ? $item->getCountry() : 'contry';
					$address = $item->getAddress() ? $item->getAddress() : 'address';
					$city = $item->getCity() ? $item->getCity() : 'city';
					if($arr_lat_1 == $post_arr_lat_1 && $arr_lng_1 == $post_arr_lng_1){
						...
						$html .= '</div></div>';
						$i++;
					}elseif(strpos($contry, $post_value) !== false || strpos($address, $post_value) !== false || strpos($city, $post_value) !== false){
						$arrayCollection['items'][] = $item->getData();
						$item->load($item->getId());
						$locationsArray[] = $item;
