...
class FilterList
{
    ...
    public function aroundGetFilters(
        ...
        if (!$this->customFilter) {
            ...
            if (($stateConfig['new_enable'] || $stateConfig['onsales_enable'] || $stateConfig['stock_enable'])
                && $this->isShowStateInCategory($stateConfig)) {
            ...
            if (isset($ratingConfig['rating_enable']) && $ratingConfig['rating_enable']
                && $this->isShowRatingInCategory($ratingConfig)) {
     ...
     * @return bool
     */
    protected function isShowRatingInCategory($ratingConfig)
    {
        if (isset($ratingConfig['categories'])) {
            $categoryIds = explode(',', $ratingConfig['categories']);
     ...
     * @return bool
     */
    protected function isShowStateInCategory($stateConfig)
    {
        if (isset($stateConfig['categories'])) {
            $categoryIds = explode(',', $stateConfig['categories']);
            if (in_array($this->getCurrentCategoryId(), $categoryIds)) {
                return false;
            }
        }
