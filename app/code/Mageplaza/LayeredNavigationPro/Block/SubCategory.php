...
class SubCategory extends Template
{
    ...
    public function getCategoryImage($category)
    {
        try {
            ...
            if (strpos($this->escapeUrl($imgUrl), $baseUrl) !== false) {
                $imgUrl = $this->escapeUrl($imgUrl);
            } else {
                $imgUrl = strstr($imgUrl, '/media/');
                if (strpos($baseUrl, '/pub/') !== false) {
                    $imgUrl = $baseUrl . $this->escapeUrl($imgUrl);
                } else {
                    $imgUrl = $baseUrl. 'pub' . $this->escapeUrl($imgUrl);
                }
            }
