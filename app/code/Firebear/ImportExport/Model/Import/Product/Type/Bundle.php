...
class Bundle extends \Magento\BundleImportExport\Model\Import\Product\Type\Bundle
{
     ...
     * Delete EmptyOptions
     */
    protected function deleteEmptyOptions()
    {
        $select = $this->connection->select()->from(
            ['bo' => $this->_resource->getTableName('catalog_product_bundle_option')],
            ['option_id']
        )->joinLeft(
            ['bos' => $this->_resource->getTableName('catalog_product_bundle_selection')],
            'bo.option_id = bos.option_id',
            ['selection_id']
        );
        ...
        if (!empty($existingOptions)) {
            foreach ($existingOptions as $existingOption) {
                if (empty($existingOption['selection_id'])) {
                    $optionsToDelete[] = $existingOption['option_id'];
                }
            }
        }
        if (!empty($optionsToDelete)) {
            $this->connection->delete(
                ...
                $this->connection->quoteInto('option_id IN (?)', $optionsToDelete)
            );
        }
    }
