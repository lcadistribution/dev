...
class ParentSku implements AdditionalFieldsInterface
{
    ...
    private function getProductIds($skus)
    {
        $select = $this->connection
            ->select()
            ->from(['cpe' => $this->connection->getTableName('catalog_product_entity')], 'entity_id')
            ->where('sku IN (?)', $skus);
    ...
    private function getParentSkus(array $childProductIds): array
    {
        ...
        $select = $this->connection
            ->select()
            ->from(['relation' => $this->connection->getTableName('catalog_product_relation')], ['child_id'])
            ...
            ->join(
                ...
                ['ev' => $this->connection->getTableName('catalog_product_entity_varchar')],
